...
namespace AOSharp.Core.Inventory
{
    public class Item : ACGItem, ICombatAction, IEquatable<Item>
    {
        ...
        public void Split(int count, bool packetOnly = false)
        {
            if (packetOnly)
            {
            ...
            N3EngineClientAnarchy.SplitItem(Slot, count);
        }
        ...
        public int GetSellPrice(Dynel vendor)
        {
            return AOMath.SellPrice(GetStat(Stat.Value), DynelManager.LocalPlayer.GetStat(Stat.ComputerLiteracy), vendor.GetStat(Stat.SellModifier) / 1000);
        }
