...
{
    public class ShadeProfessionHandler : GenericProfessionHandler
    {
        private readonly string PluginDirectory;

        public ShadeProfessionHandler(string pluginDir) : base(pluginDir)
        {
            try
            {
                ...
                Chat.WriteLine("Shade Profession Handler Loaded!");
        ...
        private void HandlerCommand(string arg1, string[] arg2, ChatWindow window)
        {
            ...
            _mainWindow = Window.CreateFromXml("PH Shade", PluginDirectory + "\\UI\\ShadeSettingsView.xml", windowStyle: WindowStyle.Default, windowFlags: WindowFlags.AutoScale | WindowFlags.NoFade);
            ...
            if (_mainWindow.FindView("SiphonLvl", out TextInputView lvl))
                lvl.Text = _settings["SpiritSiphonLvl"].AsInt32().ToString();

            if (_mainWindow.FindView("SaveLVL", out Button SaveLVL))
                SaveLVL.Clicked = Save_Lvl_Button_Clicked;
        ...
        private void Save_Lvl_Button_Clicked(object sender, ButtonBase e)
        {
            try
            {
                if (_mainWindow.FindView("SiphonLvl", out TextInputView lvl))
                {
                    if (int.TryParse(lvl.Text, out int value))
                        _settings["SpiritSiphonLvl"] = value;
                }
