...
{
    public class FixerProfessionHandler : GenericProfessionHandler
    {
        private static string PluginDirectory;

        private double _lastBackArmorCheckTime = Time.AONormalTime;

        public FixerProfessionHandler(string pluginDir) : base(pluginDir)
        {
            try
            {
                ...
                Chat.WriteLine("Fixer Profession Handler Loaded!");
        ...
        private void HandlerCommand(string arg1, string[] arg2, ChatWindow window)
        {
            ...
            _mainWindow = Window.CreateFromXml("PH Fixer", PluginDirectory + "\\UI\\FixerSettingsView.xml", windowStyle: WindowStyle.Default, windowFlags: WindowFlags.AutoScale | WindowFlags.NoFade);
        ...
        {
            if (_settings["BulletsSelection"] == null) return;
            ...
            {
                case 1:
                    bullets = Inventory.Items.FirstOrDefault(item => RelevantGenericItems.ClusterBullets.Contains(item.Id));
                    bulletsToCombine = Inventory.Items.FirstOrDefault(item => RelevantGenericItems.HomingPermorphaBullets.Contains(item.Id));
                    break;
                case 2:
                    bullets = Inventory.Items.FirstOrDefault(item => RelevantGenericItems.HomingPermorphaBullets.Contains(item.Id));
                    bulletsToCombine = Inventory.Items.FirstOrDefault(item => RelevantGenericItems.ClusterBullets.Contains(item.Id));
                    break;
                case 3:
                    bullets = Inventory.Items.FirstOrDefault(item => RelevantGenericItems.HomingPermorphaBullets.Contains(item.Id)
                    || RelevantGenericItems.ClusterBullets.Contains(item.Id));
                    break;
                default:
                    break;
        ...
        {
            if (_settings["ArmorSelection"] == null) return;
            ...
            {
                case 1:
                    if (Time.AONormalTime - _lastBackArmorCheckTime > 6)
                    {
                        _lastBackArmorCheckTime = Time.AONormalTime;
                        Item backArmor = Inventory.Items.FirstOrDefault(x => RelevantGenericItems.ShadowwebSpinner.Contains(x.HighId));
                ...
                case 2:
                    ...
                    {
                        _lastBackArmorCheckTime = Time.AONormalTime;
                        Item backArmor = Inventory.Items.FirstOrDefault(x => RelevantGenericItems.Grid.Contains(x.HighId));
