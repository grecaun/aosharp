...
{
    public class TraderProfessionHandler : GenericProfessionHandler
    {
        private readonly string PluginDirectory;

        private Window _petWindow;

        public TraderProfessionHandler(string pluginDir) : base(pluginDir)
        {
            try
            {
                ...
                _buttonDefinitions.Add(("Pets", (s, b) => _petWindow = Generic_Button_Clicked(_petWindow, "Pets", $"{PluginDirectory}\\UI\\TraderPetsView.xml")));

                #endregion

                Network.N3MessageReceived += N3MessageReceived;

                Chat.WriteLine("Trader Profession Handler Loaded!");
        ...
        private void Recieved_RootSnare_Message(int arg1, IPCMessage message)
        {
            ...
            var spell = Spell.List.Where(x => new[] { NanoLine.Root}.Contains(x.Nanoline)).OrderByStackingOrder().FirstOrDefault();
        ...
        private void HandlerCommand(string arg1, string[] arg2, ChatWindow window)
        {
            ...
            _mainWindow = Window.CreateFromXml("PH Trader", PluginDirectory + "\\UI\\TraderSettingsView.xml", windowStyle: WindowStyle.Default, windowFlags: WindowFlags.AutoScale | WindowFlags.NoFade);
        ...
        protected override void OnUpdate(float deltaTime)
        {
            try
            {
                ...
                if (DynelManager.LocalPlayer.Pets.Any(p => p.Character != null))
                    SyncPetCombat();
            ...
            foreach (var pet in DynelManager.LocalPlayer.Pets)
            {
                ...
                if (pet.Character.Velocity != 0)
                {
                    ...
                    if (target != null)
                    {
                        ...
                        else if (pet?.Character.FightingTarget.Identity != target.Identity)
                            pet?.Attack(target.Identity);
                    }
                }
            }
