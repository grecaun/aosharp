...
{
    class EnfProfessionHandler : GenericProfessionHandler
    {
        private readonly string PluginDirectory;

        public EnfProfessionHandler(string pluginDir) : base(pluginDir)
        {
            try
            {
                ...
                Chat.WriteLine("Enforcer Profession Handler Loaded!", ChatColor.Green);
                Chat.WriteLine("/handler to open or close UI", ChatColor.Green);
            }
            catch (Exception ex)
            {
                ErrorCatch(ex);
        ...
        private void HandlerCommand(string arg1, string[] arg2, ChatWindow window)
        {
            ...
            _mainWindow = Window.CreateFromXml("PH Enfo", PluginDirectory + "\\UI\\EnforcerSettingsView.xml", windowStyle: WindowStyle.Default, windowFlags: WindowFlags.AutoScale | WindowFlags.NoFade);
