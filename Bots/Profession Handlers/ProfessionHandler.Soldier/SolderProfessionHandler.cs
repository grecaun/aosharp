...
{
    public class SoldProfessionHandler : GenericProfessionHandler
    {
        private readonly string PluginDirectory;
        
        public SoldProfessionHandler(string pluginDir) : base(pluginDir)
        {
            try
            {
                ...
                Chat.WriteLine("Soldier Profession Handler Loaded!");
        ...
        private void HandlerCommand(string arg1, string[] arg2, ChatWindow window)
        {
            ...
            _mainWindow = Window.CreateFromXml("PH Soldier", PluginDirectory + "\\UI\\SoldierSettingsView.xml", windowStyle: WindowStyle.Default, windowFlags: WindowFlags.AutoScale | WindowFlags.NoFade);
            ...
            if (_mainWindow.FindView("ChannelID", out DropdownMenu chatChannels))
            {
                for (int i = ChatChannels.Count - 1; i >= 0; i--)
                    chatChannels.DeleteItem((uint)i);

                foreach (var channel in ChatChannels)
                    chatChannels.AppendItem(channel.Key.ToString());
            }
        ...
        protected override void OnUpdate(float deltaTime)
        {
            try
            {
                ...
                if (_settings["Print_AMS"].AsBool())
                {
                    if (_mainWindow?.IsValid == true)
                    {
                        _mainWindow.FindView("ChannelID", out DropdownMenu ChannelDropdown);

                        var selectionName = ChannelDropdown.GetItemLabel(ChannelDropdown.GetSelection());

                        if (selectionName != null)
                            if (Enum.TryParse(selectionName, out LocalGroupMessageType type) && ChatChannels.TryGetValue(type, out var channelId) && channelId != ChatChannelId)
                            {
                                ChatChannelId = channelId;
                                ChatChannelType = (byte)type;
                            }
                    }
                    if (DynelManager.LocalPlayer.Buffs.Find(SpellID.UserReflectShieldSoldier, out Buff AMS))
                    {
                        switch (AMS.RemainingTime)
                        {
                            case 60:
                                if (AMSPrintStop != 60)
                                {
                                    SendChannelMsg((GroupMessageType)ChatChannelType, ChatChannelId, "AMS has 60 seconds remaining");
                                    AMSPrintStop = 60;
                                }
                                break;
                            case 15:
                                if (AMSPrintStop != 15)
                                {
                                    SendChannelMsg((GroupMessageType)ChatChannelType, ChatChannelId, "AMS has 15 seconds remaining");
                                    AMSPrintStop = 15;
                                }
                                break;
                            case 0:
                                if (AMSPrintStop != 0)
                                {
                                    SendChannelMsg((GroupMessageType)ChatChannelType, ChatChannelId, "AMS EXPIRED");
                                    AMSPrintStop = 0;
                                }
                                break;
                        }
