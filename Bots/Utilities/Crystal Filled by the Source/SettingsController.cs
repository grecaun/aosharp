            ...
            if (!IsCommandRegistered)
            {
                Chat.RegisterCommand("source", (string command, string[] param, ChatWindow chatWindow) =>
                {
                    try
                    {
                        ...
                        settingsWindow = Window.Create(new Rect(50, 50, 300, 300), "Crystal Filled by the Source", "Settings", WindowStyle.Default, WindowFlags.AutoScale);

                        if (settingsWindow != null && !settingsWindow.IsVisible)
                        {
                            AppendSettingsTab("Crystal Filled by the Source", settingsWindow);
